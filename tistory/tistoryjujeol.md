# 주절주절
<b><a name="article"></a>주의! 이 페이지의 언급은 가급적이면 [저작권자](https://github.com/3-24)의 [허락](mailto:yschoelife@gmail.com)을 받은 후 해주세요.</b>

### 180428
1. Ok, Google

    구글 어시스턴트가 생각보다 쓸만하다. 음성을 인식하면서 켜지게 설정하면 보안은 일단 포기하게 되지만, 간단한 타이머나 알람은 복잡한 조작을 하지 않고 말로만 설정할 수 있다. 그리고 IFTTT로 커스터마이징(!)이 가능하기 때문에 기존의 기능을 확장해서 원하는 커맨드를 만들 수도 있다.

2. Drop the Anchor!

    anchor는 cross-referencing에 사용되는, 페이지 안에서 특정 위치를 링크를 만들어 저장하는 기능이다. github markdown의 heading에 마우스를 갖다대면 옆에 뜨는 위치링크가 포함된 그림이 뭔지 몰랐는데, 생각해보니 anchor(닻)에 연결된 사슬이더라.. 예전에 블로그에 목차를 넣을 때 쓴 적이 있던 ```<a>``` 태그도 같은 맥락이다.

    이 기능을 활용하여 티스토리 미리보기에 페이지 링크가 뜨지 않도록 수정했다.

3. (Untitled)
    
    개발일기 수준으로 이 페이지가 변해가서 걱정이다. 나는 사회적인 이슈에 대해 의견을 공개적으로 적는 것을 기피하는데, 이 부분을 고쳐나갈 것을 고려하고 있다.
...
2. 노트북을 제출해서 모바일로 마크다운을 편집하고 있는데 생각보다 효율적이다. 아마 html이었다면 '<,>'를 계속 찾아서 타이핑했겠지 ㅎ.. 는 tab이 작동 안해서 취소
